...
def train(args, dataloader, target_dataloader, netG, netD, optimizer_G, optimizer_D, netG_ema, 
    ...
    for idx in pbar:
        ...
        with torch.no_grad():
            yhat_cfeat_x = netEC(yhat, get_feature=True)
        yhat_cycle_x, _ = netG(yhat_cfeat_x, x)
        ...
        Lcycle = F.l1_loss(yhat_cycle_x, x)
        ...
        loss_dict['msk'] = Lmsk
        loss_dict['rec'] = Lrec
        loss_dict['cycle'] = Lcycle
        if args.use_idloss:
            loss_dict['id'] = Lid

        g_loss = Lgadv + Lcon + Lsty + Lmsk + Lrec + Lid + Lcycle
